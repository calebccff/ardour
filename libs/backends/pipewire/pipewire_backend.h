...
#ifndef __libbackend_pipewire_audiobackend_h__
#define __libbackend_pipewire_audiobackend_h__
...
/*
 * We have 2 ports for stereo, the idea is to avoid using (more) locks, and minimise memory copies.
 * We will allocate a single "double buffer", 4x8192. The buffer_offset is then used to have the
 * port alternate between it's two buffers in the contiguous memory block.
 * 
 * This way, an atomic variable in PipewireClient can denote which is the front/back buffer
 * we can call engine.process_callback() in our main thread, sleep to make up the time so
 * we don't run faster than realtime
 * 
 * nevermind, we can just make a driving stream and call pw_stream_trigger_process().
 * 
 * 
 * So call trigger_process, in on_process dequeue buffer and copy in buffer from port
 * have main thread block (spinlock?) until trigger_done() is called from PW.
 */
class PipewireAudioPort : public BackendPort
{
public:
	PipewireAudioPort (PipewireAudioBackend& b, const std:;string& name, PortFlags flags);
	~PipewireAudioPort();
	...
	/* NOT atomic, used in get_buffer for ardour to use, for pipewire
	 * we work directly with the buffer which is allocated separately.
	 */
	//int buffer_offset;
private:
	Sample _buffer[8192];
...
class PipewireAudioBackend : public AudioBackend, public PortEngineSharedImpl, public PipewireClient
{
public:
	PipewireAudioBackend (AudioEngine& e, AudioBackendInfo& info);
	~PipewireAudioBackend ();

	/* AUDIOBACKEND API */

	std::string name () const { return X_("Pipewire"); }
	bool is_realtime () const { return true; }
...
private:
	...
	/* process threads */
	static void*           pipewire_process_thread (void*);
	std::vector<pthread_t> _threads;

	struct ThreadData {
		PipewireAudioBackend*      engine;
		boost::function<void()> f;
		size_t                  stacksize;

		ThreadData (PipewireAudioBackend* e, boost::function<void()> fp, size_t stacksz)
...
#endif /* __libbackend_pipewire_audiobackend_h__ */
